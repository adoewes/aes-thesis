...
def quadratic_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    ...
    e = np.zeros(shape=(num_ratings,num_ratings))
    w = np.zeros(shape=(num_ratings,num_ratings))
    #print("num_ratings :", num_ratings)
    
    #w = np.array([[0,1,4], [1,0,1], [4,1,0]])
    #w = np.array([[0,0.25,1], [0.25,0,0.25], [1,0.25,0]])
    ...
    for i in range(num_ratings):
        for j in range(num_ratings):
            ...
            e[i][j] = expected_count
            
            d = pow(i - j, 2.0) / pow(num_ratings - 1, 2.0)
            
            w[i][j] = d
            
            numerator += w[i][j] * conf_mat[i][j] / num_scored_items
            denominator += w[i][j] * expected_count / num_scored_items          
    ...
    #return (qwk, e, w, np.array(conf_mat))
    return qwk


def onehalf_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    """
    Calculates the 1.5 weighted kappa
    quadratic_weighted_kappa calculates the quadratic weighted kappa
    value, which is a measure of inter-rater agreement between two raters
    that provide discrete numeric ratings.  Potential values range from -1
    (representing complete disagreement) to 1 (representing complete
    ...
    for i in range(num_ratings):
        for j in range(num_ratings):
            expected_count = (hist_rater_a[i] * hist_rater_b[j]
                              / num_scored_items)
            d = pow(abs(i - j), 1.5) / pow(num_ratings - 1, 1.5)            
            w[i][j] = d
