...
import ease.util_functions as util_functions
...
class FeatureExtractor(object):
    ...
    def initialize_dictionaries(self, e_set, max_feats2 = 200):
        ...
        if(hasattr(e_set, '_type')):
            if(e_set._type == "train"):
                ...
                # USING SAVED COUNTVECTORIZER FROM THE WHOLE ESSAYS IN DATASET
                #self._normal_dict = joblib.load("aestool/ease/normal_dict_vectorizer_asap6")

                #dictionary trained on proper vocab
                self._stem_dict = CountVectorizer(ngram_range=(1,2), vocabulary=svocab)
                # USING SAVED COUNTVECTORIZER FROM THE WHOLE ESSAYS IN DATASET
                #self._stem_dict = joblib.load("aestool/ease/stem_dict_vectorizer_asap6")
    ...
    def _get_grammar_errors(self,pos,text,tokens):
        ...
        for i in range(0, len(text)):
            ...
            if (len(pos_ngrams)-len(overlap_ngrams))>0:
                # Division in python3 that returns int (floored)
    ...
    def gen_length_feats(self, e_set):
        ...
        valid_word_counts = [max(len(t),1) for t in e_set._valid_tokens]       

        #print("Tokens : ", e_set._tokens)
        #print("Valid Tokens : ", e_set._valid_tokens)

        valid_lengths = [len(''.join(s)) for s in e_set._valid_tokens]
        #invalid_lengths = [len(''.join(s)) for s in e_set._invalid_tokens]
        #valid_lengths = [i-j for (i,j) in zip(lengths,invalid_lengths)]

        # print("Tokens : ", len(e_set._tokens))
        # print("Valid Tokens : ", len(e_set._valid_tokens))
        ...
        valid_chars_per_word = [valid_lengths[m] / float(valid_word_counts[m]) for m in range(0, len(text))]

        #print("Avg char per word : ", chars_per_word)
        #print("Valid Avg char per word : ", valid_chars_per_word)
        ...
        length_arr = numpy.array((
        lengths, word_counts, comma_count, ap_count, punc_count, valid_chars_per_word, good_pos_tags,
        good_pos_tag_prop)).transpose()

        return length_arr.copy()

    #This is addition from this paper : "AUTOMATED ESSAY SCORING LINGUISTIC FEATURE: COMPARATIVE STUDY"
    def gen_addition_feats(self, e_set):

        text = e_set._text
        cleaned_text = e_set._clean_text
        ...
        fourth_root_wc = [wc ** (1./4.) for wc in word_counts]

        question_count = [e.count("?") for e in text]
        exclamation_count = [e.count("!") for e in text]

        spelling_errors = [se for se in e_set._spelling_errors]

        wc_more_than_5 = [len([elem for elem in t if len(elem) > 5]) for t in e_set._tokens]
        wc_more_than_6 = [len([elem for elem in t if len(elem) > 6]) for t in e_set._tokens]
        wc_more_than_7 = [len([elem for elem in t if len(elem) > 7]) for t in e_set._tokens]
        wc_more_than_8 = [len([elem for elem in t if len(elem) > 8]) for t in e_set._tokens]        

        unique_word_counts = [len(set(t)) for t in e_set._tokens]

        stop_words = set(stopwords.words('english')) 

        stopword_count = [len([w for w in t if w in stop_words]) for t in e_set._tokens]

        sentence_count = [len(sent_tokenize(t)) for t in text]

        wc = numpy.array(word_counts).astype(numpy.float32)
        sc = numpy.array(sentence_count).astype(numpy.float32)

        avg_sentence_length = wc / sc #for each essay        

        #sum of word counts for the whole essay_set
        wc_sum = numpy.sum(wc)

        #sum of char_counts for the whole essay_set
        cl = numpy.array(lengths)
        cl_sum = numpy.sum(cl)

        #The average word length over the whole training data
        avg_word_length = cl_sum / wc_sum

        #Long word count : the number of words whose length is greater than average word length of the training data (AWCtot)

        long_word_counts = [len([w for w in t if len(w) > avg_word_length]) for t in e_set._tokens]       

        #Average word length
        chars_per_word = [lengths[m] / float(word_counts[m]) for m in range(0, len(text))]

        noun_count = [len([tagged_words for tagged_words in nltk.pos_tag(t) if tagged_words[1].startswith('N')]) for t in e_set._tokens]
        verb_count = [len([tagged_words for tagged_words in nltk.pos_tag(t) if tagged_words[1].startswith('V')]) for t in e_set._tokens]
        adj_count = [len([tagged_words for tagged_words in nltk.pos_tag(t) if tagged_words[1].startswith('J')]) for t in e_set._tokens]
        adv_count = [len([tagged_words for tagged_words in nltk.pos_tag(t) if tagged_words[1].startswith('R')]) for t in e_set._tokens]

        # for s in e_set._tokens:
        #     print(s)
        #     print(len(s))
        #     print(len(set(s)))
        #     print([w for w in s if w in stop_words])

        # for e in cleaned_text:
        #     print(e)


        difficult_words_list = joblib.load("../SAT_5000_words")

        difficult_words_count = [len([w for w in t if w in difficult_words_list]) for t in e_set._tokens]


        addition_arr = numpy.array((
            lengths, word_counts, comma_count,
            fourth_root_wc, question_count, exclamation_count, spelling_errors, 
            wc_more_than_5, wc_more_than_6, wc_more_than_7, wc_more_than_8, 
            unique_word_counts, stopword_count,sentence_count, 
            noun_count, verb_count, adj_count, adv_count,
            chars_per_word, avg_sentence_length, long_word_counts, difficult_words_count)).transpose()

        return addition_arr.copy()
    ...
    def gen_bag_feats(self, e_set):
        ...
        if(hasattr(self, '_stem_dict')):
            ...
            print("stem bag: ", sfeats.shape)            
            e_set._sfeats_size = sfeats.shape[1]

            nfeats = self._normal_dict.transform(e_set._text)
            
            print("normal bag: ", nfeats.shape)
            e_set._nfeats_size = nfeats.shape[1]
    ...
    #     """
    #     bag_feats = self.gen_bag_feats(e_set)
    #     print("bag : ", bag_feats.shape)
    #     length_feats = self.gen_length_feats(e_set)
    #     print("length : ", length_feats.shape)
    #     prompt_feats = self.gen_prompt_feats(e_set)
    #     print("prompt : ", prompt_feats.shape)
    #     print(prompt_feats)

    #     overall_feats = numpy.concatenate((length_feats, prompt_feats, bag_feats), axis=1)
    #     overall_feats = overall_feats.copy()

    #     print("overall : ", overall_feats.shape)

    #     return overall_feats


    #This is modified version which includes prompt addition
    def gen_feats(self, e_set, prompt):
        """
        Generates bag of words, length, and prompt features from an essay set object
        returns an array of features
        e_set - EssaySet object
        ...
        ###############################################################################################################
        ## In this version, we want to IGNORE the BAG_OF_WORDS feature from EASE. We only use the first 12 features. ##
        ###############################################################################################################

        #bag_feats = self.gen_bag_feats(e_set)
        #print("bag : ", bag_feats.shape)

        #add_feats = self.gen_addition_feats(e_set)
        #print("addition : ", add_feats.shape)

        spelling_errors = [se for se in e_set._spelling_errors]
        spelling_feats = numpy.array(spelling_errors).reshape(len(e_set._spelling_errors),-1)

        #print("spelling : ", spelling_feats.shape)

        unique_word_counts = [len(set(t)) for t in e_set._valid_tokens]
        unique_feats = numpy.array(unique_word_counts).reshape(len(e_set._valid_tokens),-1)

        #print("unique words : ", unique_feats.shape)

        #overall_feats = numpy.concatenate((length_feats, prompt_feats, bag_feats), axis=1)
        #overall_feats = numpy.concatenate((length_feats, prompt_feats, bag_feats, spelling_feats, unique_feats), axis=1)
        overall_feats = numpy.concatenate((length_feats, prompt_feats, spelling_feats, unique_feats), axis=1)
        
        overall_feats = overall_feats.copy()        
        #overall_feats = add_feats.copy()        
