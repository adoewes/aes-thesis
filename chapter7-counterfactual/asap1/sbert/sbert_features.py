...
def create_sbert_features(answer):
    ...
    #model_filename = os.path.join(script_dir, '../ease/', model_name)
	...
	#model = SentenceTransformer('/home/afrizal/Documents/Python Scripts/AES_3/bert-large-nli-stsb-mean-tokens')

	#print("Answer before preprocess: \n", answer)

	answer = preprocess_text(answer, True, False, False)

	#print("Answer after preprocess: \n", answer)

	sentence_list = sent_tokenize(answer)

	#print("\n", sentence_list)
	
	sentence_embeddings = model.encode(sentence_list)
	
	mean_sentence_embedding = np.average(sentence_embeddings, axis=0)

	# Encode the answer to Sentence-Bert (768 dim)	
	
	#print("Embedding:", mean_sentence_embedding)
	#print("Embedding dim : ", len(mean_sentence_embedding))

	return mean_sentence_embedding


def preprocess_text(raw_text, remove_nonletters, remove_stopwords, stemming):

    # 1. Remove HTML
    text = BeautifulSoup(raw_text, 'lxml').get_text()

    # 2. Remove non-ASCII
    letters_only = re.sub(r"[^\x00-\x7F]+", " ", text)
    ...
    # 7. Join words back into one string by space, and return the result
    return(" ".join(words))