...
def get_vocab(text, score, max_feats=750, max_feats2=200):
    ...
    set_score = numpy.asarray(score, dtype=numpy.int64)
    ...
    vocab = getVar(dict.get_feature_names_out(), good_cols)
...
def gen_cv_preds(clf, arr, sel_score, num_chunks=3):
    ...
    preds = []
    set_score = numpy.asarray(sel_score, dtype=numpy.int64)
...
def get_wordnet_syns(word):
    ...
    for ss in synset:
        #for swords in ss.lemma_names: --> original, gives error
