...
class PredictorExtractor(object):
    ...
    def gen_feats(self, p_set, prompt):
        """
        Generates features based on an iput p_set
        p_set - PredictorSet
        """
        ...
        for i in range(0,len(p_set._essay_sets)):
            textual_features.append(self._extractors[i].gen_feats(p_set._essay_sets[i], prompt))
        ...
        stem_bag_size = p_set._essay_sets[0]._sfeats_size

        norm_bag_size = p_set._essay_sets[0]._nfeats_size

        return overall_matrix.copy(), stem_bag_size, norm_bag_size
